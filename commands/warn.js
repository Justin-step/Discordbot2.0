...
module.exports.run = async (client, message, args) => {
    ...
    var warnUser = message.guild.members.cache.get(message.mentions.users.first().id || message.guild.get(args[0]).id)

    var reason = args.slice(1).join(" ");

    if(!warnUser) return message.reply("Kan gebruiker niet vinden");

    if(warnUser.permissions.has("MANAGE_MESSAGES")) return message.reply("Sorry u kunt deze persoon geen waarschuwing geven");
...
module.exports.help = {
    name: "warn",
    category: "general",
    description: "Waarschuwt een persoon"
}